...
export default function ProductsGrid() {
 ...
 const getsOrderTemplates = () => {
  let config = {
    method: "get",
    maxBodyLength: Infinity,
    url: "http://127.0.0.1:8000/ordertemp",
    headers: {},
  };
  ...
  axios
    ...
    .then((response) => {
      ...
      setTemplateCount(templates.length); // update count
console.log(templates.length)
      if (templates.length === 0) {
        setOpen(true); // open dialog if templates exist
      }
    })
    .catch((error) => {
      console.log(error);
...
useEffect(() => {
  ...
  const handleYesChange = () => {
    toast.success("Creating new purchase template");
    toast.success("Please select category");
  ...
  return (
    <>
              ...
              {row.map((product, index) => (
                ...
                >
                  <Card
                    ...
                    onClick={() => navigate(`${product.route}/${product.slug}`)}
                    sx={{
      ...
      >
        <DialogTitle id="alert-dialog-title">{"Purchase Template"}</DialogTitle>
        <DialogContent>
          <DialogContentText id="alert-dialog-description">
            Do you want to create template?
          </DialogContentText>
        </DialogContent>
        <DialogActions>
          <Button onClick={handleYesChange} autoFocus>
            Yes
          </Button>
