...
import bekariImage from "../assets/bekari.png";
import dairyImage from "../assets/dairy.png";
import namkinImage from "../assets/namakin.png";
// import hanumanImage from "../assets/advertizement.png";
import cakeImage from "../assets/cakes.png";
import takImage from "../assets/advertizement.png";
import laddu from "../assets/advertizement.png";
...
const testimonials = [
  {
    ...
    feedback: "स्वस्त दरात चांगले उत्पाद आहे. नवीन त्यात उपलब्ध करून देत आहेत त्यामुळे खरेदीसाठी वापरायला योग्य आहे.",
    rating: 5,
    image: profilePic,
  },
  {
...
export default function ProductsGrid() {
  ...
  return (
    <>
      <Grid container spacing={1} border={"3px solid blue"}>
        <Grid size={{ xs: 12, md: 8 }} sx={{ p: 1, border: "2px solid red" }}>
          {rows.map((row, rowIndex) => (
            <Grid container spacing={2} key={rowIndex}>
              {row.map((product, index) => (
                <Grid size={{ xs: 12, md: 6 }} key={index} sx={{ flexWrap: 'wrap' }}>
                  <Card
                  ...
                  >
                    ...
                    <CardContent sx={{ p: 0 }}>
                      ...
                      <Tooltip title={product.products} sx={{ cursor: 'pointer' }} placement="right-start">
                        <Typography variant="body2" sx={{ color: 'text.secondary' }} fontWeight="700">
                          {product.products.split(",").slice(0, 5).join(", ") + "..."}
                        </Typography>
                      </Tooltip>

                      <Tooltip title={product.productsMarathi} sx={{ cursor: 'pointer' }} placement="right-start">
                        <Typography variant="body2" sx={{ color: 'text.secondary' }} fontWeight="700">
                          {product.productsMarathi.split(",").slice(0, 5).join(", ") + "..."}
                        </Typography>
                      </Tooltip>
                    </CardContent>
                  </Card>
      ...
      <Grid container spacing={2} border={'3px solid green'} width={"100%"} sx={{ p: 1 }}>
        <Grid size={{ xs: 12, md: 8 }}>
          ...
          >
            {ceoMessages.map((msg, i) => (
              ...
              >
                <Typography fontWeight={700} mb={1}>{msg.heading1}</Typography>
                <Typography fontWeight={700} mb={1}>{msg.heading2}</Typography>
                <Typography fontWeight={700} mb={2}>{msg.heading3}</Typography>

                <Box display="flex" alignItems="center" justifyContent="space-between">
                  <Box component="img" src={cowImage} alt="cows" sx={{ width: 200 }} />
